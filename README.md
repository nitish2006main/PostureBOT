...
How it works
The computer turns on the webcam and looks for your face every
frame, tracking the tip of your nose.
It figures out how far off-center your nose is and sends that to the
ESP32 about 20 times a second.
The ESP32 uses that info to turn the motors and keep the camera
pointed at your face.
It also looks at a few more points on your face (eyes, mouth, chin)
to estimate which way your head is turned or tilted.
Based on how big your face looks in the frame, it estimates how far
away you are from the screen.
Calibration: you hold your face still in a box on screen for 5
seconds. That spot becomes "home base" — the ESP32 remembers this
even after a restart.
As you move around, the ESP32 keeps sending back your position (in
centimeters) compared to that home base, and the computer logs it.
If your face disappears or your eyes close for too long (5 seconds),
...
When the session ends, the computer draws a 3D graph showing how your
...
Replay — see the 3D graph for one specific session again.
Trend — see a score (0–100%) for every session over time, with
arrows showing whether each session was better or worse than the last
one.
Overlay — pick 2 or more sessions and see their movement paths
...
How to set it up
Firmware (ESP32 side)
Install the Arduino IDE and add ESP32-S3 board support.
Install these libraries: Adafruit SSD1351, Adafruit GFX,
Adafruit PWMServoDriver (the Preferences library comes built in).
Wire everything up using the pin table above.
Upload posturebot_v4.ino to the board.
Python (computer side)
bash
pip install opencv-python mediapipe pyserial matplotlib numpy
Set which USB port to use in face_tracker_v4.py (or just let it
